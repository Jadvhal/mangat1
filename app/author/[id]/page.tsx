...
export default async function AuthorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-3xl font-bold dark:text-white text-black">{authorName}</h1>
      ...
      {mangaList.length > 0 && (
        <div className="flex justify-center items-center gap-4 mt-8">
          <button className="flex items-center gap-2 px-4 py-2 dark:bg-white/5 bg-black/5 dark:hover:bg-white/10 hover:bg-black/10 rounded-lg text-sm dark:text-zinc-400 text-zinc-600 transition-colors">
            <ChevronLeft className="w-4 h-4" />
            Previous
          </button>
          <div className="dark:bg-white bg-black dark:text-black text-white w-8 h-8 flex items-center justify-center rounded-lg text-sm font-bold">
            1
          </div>
          <button className="flex items-center gap-2 px-4 py-2 dark:bg-white/5 bg-black/5 dark:hover:bg-white/10 hover:bg-black/10 rounded-lg text-sm dark:text-zinc-400 text-zinc-600 transition-colors">
            Next
